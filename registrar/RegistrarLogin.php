...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
    header('Location: RegistrarHome.php'); // Redirect to the dashboard or another page
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query to check if the user exists in the database and is in the Registrar office
    $sql = "SELECT * FROM user_accounts WHERE username = '$username' AND password = '$password' AND office = 'Registrar'";
    ...
    if ($result->num_rows == 1) {
        ...
        header('Location: RegistrarHome.php'); // Redirect to a dashboard page
    } else {
        // Invalid credentials or wrong office
        $message = "Login failed. Please check your username, password, or office.";
        echo '<script>alert("' . $message . '"); window.location.href="RegistrarLogin.php";</script>';
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-8 p-5">
                <h4 class="fst-italic fs-3 p-5 fw-bold text-center nu_color">Registrar Office.</h4>
              
<!-- log in page goes here-->

<div class="login">
    <form id="login" method="post" action="RegistrarLogin.php">
