...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['office'])) {
    ...
    if ($formattedSelectedOffice === 'Academics') {
        ...
        if ($courseResult->num_rows > 0) {
            ...
            VALUES ('$selectedQueueNumber', '$selectedTimestamp', '$selectedStudentID', '$selectedProgram', '$selectedConcern', '$selectedCourse', '$selectedRemarks', 'Registrar', '$selectedTransaction')";
        ...
        VALUES ('$selectedQueueNumber', '$selectedTimestamp', '$selectedStudentID', '$selectedRemarks', 'Registrar', '$selectedTransaction')";
        ...
        $sqlInsert = "INSERT INTO $tableName (queue_number, timestamp, student_id, remarks, endorsed_from, transaction) VALUES ('$selectedQueueNumber', '$selectedTimestamp', '$selectedStudentID', '$selectedRemarks', 'Registrar', '$selectedTransaction')";
...
// Fetch endorsed_from value from the Registrar table
$sqlFetchEndorsedFrom = "SELECT endorsed_from FROM registrar WHERE queue_number = '$selectedQueueNumber'";
...
if ($resultFetchEndorsedFrom->num_rows > 0) {
    ...
    // Insert data into registrar_logs table
    $sqlInsertIntoAccountingLogs = "INSERT INTO registrar_logs (queue_number, timestamp, student_id, remarks, endorsed_from, transaction)
    VALUES ('$selectedQueueNumber', '$selectedTimestamp', '$selectedStudentID', '$selectedRemarks', '$selectedEndorsedFrom', '$selectedTransaction')";

    // Execute the insert query for registrar_logs table
    if ($conn->query($sqlInsertIntoAccountingLogs) === TRUE) {
        // If insert is successful, update the 'endorsed_to' column in the 'registrar_logs' table
        $sqlUpdateEndorsedToLogs = "UPDATE registrar_logs SET endorsed_to = '$formattedSelectedOffice' WHERE queue_number = '$selectedQueueNumber'";

        // Execute the update query for the 'registrar_logs' table
        if ($conn->query($sqlUpdateEndorsedToLogs) !== TRUE) {
            echo "Error updating endorsed_to column in registrar_logs: " . $conn->error;
        }

        // Update status column in the 'registrar' table and display table
        $sqlUpdateStatusAndDisplay = "UPDATE registrar, display
        SET registrar.status = 1,
            display.status = 1
        WHERE registrar.queue_number = '$selectedQueueNumber'
            AND display.queue_number = '$selectedQueueNumber'
            AND display.officeName = 'Registrar'";

        if ($conn->query($sqlUpdateStatusAndDisplay) !== TRUE) {
            echo "Error updating status and display table: " . $conn->error;
        }

        // Delete the data from the 'registrar' table
        $sqlDeleteFromAccounting = "DELETE FROM registrar WHERE queue_number = '$selectedQueueNumber'";

        // Execute the delete query for the 'registrar' table
        if ($conn->query($sqlDeleteFromAccounting) !== TRUE) {
            echo "Error deleting data from registrar table: " . $conn->error;
        }
        ...
        VALUES ('$selectedQueueNumber', '$selectedStudentID', 'Registrar', '$selectedRemarks', '" . ucfirst($selectedOffice) . "')";
        ...
        echo "Error inserting data into registrar_logs table: " . $conn->error;
    }
} else {
...
// Fetch data from the "registrar" table where status is 0 and availability is 0, ordered by timestamp in descending order
$sqlAccounting = "SELECT queue_number, timestamp, student_id, transaction, endorsed_from, remarks FROM registrar WHERE status = 0 AND availability = 0 ORDER BY timestamp DESC";
...
// Fetch data from the "queue" table where office is "registrar" and status is 0, ordered by timestamp in descending order
...
// Combine the data into a single array, with data from "registrar" at the top
...
if (isset($_POST['update_button'])) {
    ...
    $sqlUpdate = "UPDATE registrar SET window = 1 WHERE queue_number = '$selectedQueueNumber'";
...
<head>
    ...
    <title>NU Registrar Office</title>
...
<div class="sidenav" id="sidenav">
    ...
    <!-- Display queue numbers from both "queue" and "registrar" tables where status is 0 -->
...
<?php if (isset($_POST['show_details'])): ?>
    ...
    $sqlUpdateAvailability = "UPDATE registrar SET availability = 1 WHERE queue_number = '$selectedQueueNumber'";
    ...
    // Fetch the "endorsed" data from the "registrar" table
    $sqlFetchEndorsed = "SELECT endorsed_from FROM registrar WHERE queue_number = '$selectedQueueNumber'";
    ...
    <div id="center" class="center-content">
        ...
        <button style="margin-left: 2%;" class="button" type="button" id="endorseButton" onclick="openEndorsementPopup()" disabled>
    ...
    <script>
        ...
        function fetchUserWindowAndNotify() {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    // Now, you can use the fetched window value to update the registrar table
                    updateRegistrarTable(userWindow);
                }
            };
        ...
        function updateRegistrarTable(userWindow) {
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "UpdateRegistrarWindow.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
...
<?php if (isset($_POST['show_details'])): ?>
    ...
    <script>
        function markAsDone() {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "RegistrarBackendScript.php", true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    console.log(xhr.responseText);
...
function submitFormPopup() {
    ...
    function updateConcernOptions(selectedProgram) {
        ...
        fetch('./RegistrarServerScript.php', {
            method: 'POST',
...
<script>
    ...
    function returnToQueue() {
        ...
        $.ajax({
            type: 'POST',
            url: 'RegistrarBackAvailability.php', // adjust the URL based on your file structure
    ...
    //logout the user
    function exit() {
        window.location.href = "RegistrarLogout.php";
    }
    ...
    $.ajax({
        url: "RegistrarHome.php", // Change to your PHP script file
        method: "POST",
